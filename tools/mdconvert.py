...
class _CustomMarkdownify(markdownify.MarkdownConverter):
    ...
    def convert_a(self, el: Any, text: str, convert_as_inline: bool = False, parent_tags=None, **kwargs) -> str:
        ...
        if (
            self.options.get("autolinks")
            and text.replace(r"\_", "_") == href
            and not title
            and not self.options.get("default_title")
        ):
            return f"<{href}>"
        if self.options.get("default_title") and not title:
            title = href
    ...
    def convert_img(self, el: Any, text: str, convert_as_inline: bool = False, **kwargs) -> str:
        # Optionally: parent_tags = kwargs.get("parent_tags")
        alt = el.attrs.get("alt", None) or ""
        src = el.attrs.get("src", None) or ""
        title = el.attrs.get("title", None) or ""
        title_part = ' "%s"' % title.replace('"', r"\"") if title else ""
    ...
    def convert_soup(self, soup: Any) -> str:
        try:
            ...
            print('EXCEPTION IN CONVERT_SOUP:', e)
...
class HtmlConverter(DocumentConverter):
    ...
    def _convert(self, html_content: str) -> None | DocumentConverterResult:
        ...
        # print('body_elm',body_elm)
        webpage_text = ""
        if body_elm:
            # print('IN BODY ELEM')
            webpage_text = _CustomMarkdownify().convert_soup(body_elm)
            # print('webpage text1', webpage_text)
        else:
            webpage_text = _CustomMarkdownify().convert_soup(soup)
            # print('webpage text2', webpage_text)
...
class WikipediaConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        ...
        if not re.search(r"^https?:\/\/[a-zA-Z]{2,3}\.wikipedia.org\/", url):
            return None
        print('WikipediaConverter', local_path)
        # Parse the file
        soup = None
        with open(local_path, "rt", encoding="utf-8") as fh:
...
class YouTubeConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        ...
        if not url.startswith("https://www.youtube.com/watch?"):
            return None
        print('YouTubeConverter', local_path)
        # Parse the file
        soup = None
        with open(local_path, "rt", encoding="utf-8") as fh:
            soup = BeautifulSoup(fh.read(), "html.parser")
...
class PptxConverter(HtmlConverter):
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        ...
        if extension.lower() != ".pptx":
            return None
        print('PptxConverter', local_path)
...
class MarkdownConverter:
    ...
    def convert_local(self, path: str, **kwargs: Any) -> DocumentConverterResult:  # TODO: deal with kwargs
        ...
        # print('convert local', path, kwargs)
    ...
    def convert_response(
        ...
        # Prepare a list of extensions to try (in order of priority)
        # print('CONVERT RESPONSEEEEEE')
        ext = kwargs.get("file_extension")
        extensions = [ext] if ext is not None else []
        # print('extention', ext, extensions)
